...
sub main {
    ...
    while (my $line = <$in_fh>) {
	  ...
	  open my $out_fh2, ">", "$outdir/$filename.other.csv";

          
          while (my $line2 = <$in_fh2>) {
                 chomp $line2;
                 my @title = split(",",$line2);
                 
                 if (looks_like_number($title[4])){  
                     if ($title[4] >= 5) {
                     
                        print $out_fh1 $filename,",",join(",",@title), "\n";
                 
                     } else {
                 
                        print $out_fh2 $filename,",",join(",",@title), "\n";
                     }
                 } 
          }
          close($in_fh2);
